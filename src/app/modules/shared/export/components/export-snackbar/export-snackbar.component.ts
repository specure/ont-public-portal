...
export class ExportSnackbarComponent {
  reports$ = this.store.select(getExportState).pipe(
    map(s => s.exportedReports ? Object.values(s.exportedReports).reverse() : [])
  )
  limitReached$ = this.store.select(getExportState).pipe(pluck('exportLimitReached'))
  ...
  download(report: IExportedReport, total: number) {
    ...
    this.dataService.downloadData(report).subscribe(res => {
      ...
      this.downloadedReports.splice( this.downloadedReports.indexOf(report.id), 1 )
  ...
  getSnackbarStatus(exportStatus?: EExportStatus): Observable<string> {
    return this.transloco.selectTranslate(`opendata.${this.data.format}.snakbar_${exportStatus.toLocaleLowerCase()}`)
  }
