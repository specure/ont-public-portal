...
export class PopupService {
  ...
  addPopup(
    ...
    return this.getPopupContent(mapContainer, e?.features[0]).pipe(
      first(),
      tap((content) => {
        if (!this.popup) {
  ...
  addMunicipalityPopupToCenter(mapContainer: Map, coordinates: LngLat) {
    return this.store.select(getMapState).pipe(
      ...
      switchMap((state) => {
        ...
        const feature = this.mapService.findRenderedFeature(
          mapContainer,
          currentLayer,
          coordinates
        )
        ...
        return this.getPopupContent(mapContainer, feature)
      }),
      tap((content) => {
        if (!this.popup) {
          this.popup = new Popup()
  ...
  editPopup(mapContainer: Map, currentLayer: string) {
    ...
    return this.getPopupContent(mapContainer, feature).pipe(
      first(),
      tap((content) => {
        if (content) {
  ...
  private getPopupContent(
    ...
    return this.getPopupPropertyPrefix().pipe(
      take(1),
      map((prefix) => {
        if (e) {
          if (
            !this.showZeroValues(mapContainer) &&
            !e.properties[prefix + '-COUNT']
          ) {
            return ''
          }

          const download = `${convertBytes(
            (e.properties[prefix + '-DOWNLOAD'] || 0) * 1000
          )
            .to(ESpeedUnits.MBPS)
            .toLocaleString()} Mbps`
          const upload = `${convertBytes(
            (e.properties[prefix + '-UPLOAD'] || 0) * 1000
          )
            .to(ESpeedUnits.MBPS)
            .toLocaleString()} Mbps`
          const ping = `${convertMs(
            e.properties[prefix + '-PING'] || 0
          ).toLocaleString()} ms`

          return [
            {
              label: this.getPopupHeader(mapContainer),
              value: e.properties.NAME || '',
            },
            {
              label: 'map.popup.count',
              value: e.properties[prefix + '-COUNT'] || 0,
            },
  ...
  private showZeroValues(mapContainer: Map) {
    ...
    return (
      prefix === ELayerPrefix.COUNTIES || prefix === ELayerPrefix.MUNICIPALITIES
    )
  }
}
